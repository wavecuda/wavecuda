...
int main(void){
  ...
  for(power = p0; power<=p; power++){
    ...
    mptimer(-1);
    for(i=0;i<reps;i++){
      res=LA8(x_h,len,FWD,levels);
      //res=lLA8(x_h,len,FWD,levels);
      if(res!=0) printf("\n## Error in transform\n");
      res=LA8(x_h,len,BWD,levels);
      //res=lLA8(x_h,len,BWD,levels);
      if(res!=0) printf("\n## Error in transform\n");
    ...
    mptimer(-1);
    for(i=0;i<reps;i++){
      res=lompLA8(x1_h,len,FWD,levels);
      if(res!=0) printf("\n## Error in transform\n");
      res=lompLA8(x1_h,len,BWD,levels);
      if(res!=0) printf("\n## Error in transform\n");
    }
    t2=mptimer(1);
    ...
    for(i=0;i<reps;i++){
      // res=LA8CUDA_sh(x2_d,len,FWD,levels); //lifting with shared memory
      res=LA8CUDA_sh_ml2(x2_d,len,FWD,levels); //lifting with shared memory
      ...
      // res=LA8CUDA_sh(x2_d,len,BWD,levels); //lifting with shared memory
      res=LA8CUDA_sh_ml2(x2_d,len,BWD,levels); //lifting with shared memory
      // cudaDeviceSynchronize();
      // cuderr = cudaGetLastError();
      // if (cuderr != cudaSuccess)
      // 	{
    ...
    ops=(double) len * (double) reps * 2. * 24 * 2.; // with reverse
